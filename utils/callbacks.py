    ...
    def __init__(self, log_dir, model, input_shape):
        ...
        try:
            ...
            if hasattr(model, "module"):
                in_ch = getattr(model.module, "in_channels", in_ch)
            dummy_input = torch.randn(2, int(in_ch), input_shape[0], input_shape[1])
...
class EvalCallback:
    ...
    def get_history_imgs(self, line):
        ...
        index = int(line.split("/")[-1][:8])
        return [
            os.path.join(dir_path, "%08d.%s" % (max(id, 1), file_type))
    ...
    def get_map_txt(self, image_id, images, class_names, map_out_path):
        ...
        # ---------------------------------------------------------#
        image_data = [
            np.transpose(preprocess_input(np.array(image, dtype="float32")), (2, 0, 1))
